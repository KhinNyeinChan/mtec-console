...
import { UserRoleListPage } from './user-role-list';

describe('UserRoleListPage', () => {
  let fixture: ComponentFixture<UserRoleListPage>;

  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [UserRoleListPage],
      providers: [provideSharedTestTranslate()],
    }).compileComponents();

    seedSharedTestTranslate(TestBed.inject(TranslateService));
    fixture = TestBed.createComponent(UserRoleListPage);
  ...
  it('should show the title, search, and dummy role rows', () => {
    ...
    expect(title.textContent).toContain('User Role List');
  ...
  it('should filter dummy rows from the simple search', () => {
    fixture.componentInstance.onSimpleSearch('cashier');
    fixture.detectChanges();

    const rows = [...fixture.nativeElement.querySelectorAll('tbody tr')] as HTMLElement[];
    expect(rows.length).toBe(1);
    expect(rows[0].textContent).toContain('Cashier');
  });
