...
import { DUMMY_USER_ROLES, UserRoleListItem } from '../models/user-role.model';

@Component({
  selector: 'app-user-role-list-page',
  standalone: true,
  imports: [TranslatePipe, SimpleSearch, AdvSearch, Table],
  templateUrl: './user-role-list.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class UserRoleListPage {
  private readonly i18n = inject(TranslateService);
  private readonly router = inject(Router);

  /** Dummy rows until the user role list API is connected. */
  private readonly source = signal<UserRoleListItem[]>(DUMMY_USER_ROLES);
  ...
  readonly columns: TableColumn<UserRoleListItem>[] = [
  ...
  goNew(): void {
    void this.router.navigateByUrl('/admin/roles/new');
  }
  ...
  private matches(row: UserRoleListItem, filter: AdvSearchRow): boolean {
