...
import { tenantIdFromHostname } from '../login/login';
...
@Component({
  selector: 'app-tenant-forgot-password-page',
  standalone: true,
...
export class TenantForgotPasswordPage {
  ...
  onEmailSubmit(): void {
    ...
    this.auth
      .requestTenantResetOtp({ tenantId, email })
  ...
  onOtpSubmit(): void {
    ...
    this.auth
      .verifyTenantResetOtp({ tenantId: this.tenantId(), email: this.email(), otp })
      .pipe(finalize(() => this.submitting.set(false)))
      .subscribe({
        next: () => {
  ...
  onPasswordSubmit(): void {
    ...
    this.auth
      ...
      .pipe(finalize(() => this.submitting.set(false)))
      .subscribe({
        next: () => void this.router.navigateByUrl('/auth/tenant/login'),
